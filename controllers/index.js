...
ngApp.controller("indexController", function ($scope, $http, $sce) {
    ...
    $scope.download = async () => {
        try {
            if ($scope.local.collect.length === 0) {
                $scope.snackbar.show("No data to download. Please extract data first.");
                return;
            }
            ...
            // Use a more efficient way to create CSV content
            const headers = columnList.join(',');
            let csvContent = headers + '\n';
    
            for (let item of $scope.local.collect) {
                csvContent += columnList.map(col => item[col] || '').join(',') + '\n';
            }
            ...
            link.href = url;
            link.setAttribute('download', 'map_extractor_logs.csv');
            ...
            $scope.snackbar.show("Download completed successfully");
        } catch (error) {
            console.error("Download failed:", error);
            $scope.snackbar.show("Download failed. Please try again.");
            // Hide download modal in case of error
            $scope.local.showDownloadModal = false;
        }
    };
...
function buildDataTable() {
    $box.getLocal(local => {
        ...
        dataGrid = $("#gridContainer").dxDataGrid({
            ...
            paging: {
                enable: true,
                pageSize: 100, // Increase this value or remove paging altogether
            },
            pager: {
                showPageSizeSelector: true,
                showNavigationButtons: true,
                allowedPageSizes: [25, 50, 100, 500, 1000], // Add larger page sizes
                showInfo: true
            },
            dataSource: {
                store: productsStore,
